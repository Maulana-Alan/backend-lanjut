...
// NewPool bikin connection pool ke PostgreSQL
// Pool = kumpulan koneksi siap pakai, gak perlu buka-tutup tiap request
func NewPool(ctx context.Context) (*pgxpool.Pool, error) {
	dsn := fmt.Sprintf(
		"postgres://%s:%s@%s:%s/%s?sslmode=%s",
		config.GetEnv("DB_USER", "ASUS"),
	...
	// Ping = cek koneksi beneran nyambung, bukan nunggu error di request pertama
